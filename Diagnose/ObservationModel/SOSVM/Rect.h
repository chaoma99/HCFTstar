...
public:
	...
	inline double XCentre() const { return (double)m_xMin + (double)m_width / 2; }
	inline double YCentre() const { return (double)m_yMin + (double)m_height / 2; }
	...
	template <typename T2>
	double Overlap(const Rect<T2>& rOther) const;
...
template <typename T>
template <typename T2>
double Rect<T>::Overlap(const Rect<T2>& rOther) const
{
	double x0 = std::max((double)XMin(), (double)rOther.XMin());
	double x1 = std::min((double)XMax(), (double)rOther.XMax());
	double y0 = std::max((double)YMin(), (double)rOther.YMin());
	double y1 = std::min((double)YMax(), (double)rOther.YMax());
	
	if (x0 >= x1 || y0 >= y1) return 0.f;
	
	double areaInt = (x1-x0)*(y1-y0);
	return areaInt/((double)Area()+(double)rOther.Area()-areaInt);
}
