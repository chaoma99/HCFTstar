...
public:
  ...
  float _maxAspectRatio, _minBoxArea,_maxBoxArea, _gamma, _kappa,_aspectRatio;
...
void EdgeBoxGenerator::scoreAllBoxes( Boxes &boxes )
{
  ...
  float maxSize=sqrt(_maxBoxArea);
  float aspect=sqrt(_aspectRatio);
  arRad = 1;//int(log(_maxAspectRatio)/log(_arStep*_arStep));
  scNum = int(ceil(log(maxSize/minSize)/log(_scStep))); //max(w,h)
  for( int s=0; s<scNum; s++ ) {
    ...
    for( a=0; a<2*arRad+1; a++ ) {
      ...
      bh=int(sc/aspect); kr=max(2,int(bh*_rcStepRatio));
      bw=int(sc*aspect); kc=max(2,int(bw*_rcStepRatio));
...
void mexFunction( int nl, mxArray *pl[], int nr, const mxArray *pr[] )
{
  ...
  edgeBoxGen._maxBoxArea = float(mxGetScalar(pr[14]));
  edgeBoxGen._aspectRatio = float(mxGetScalar(pr[15]));
